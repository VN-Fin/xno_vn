...
class StockAlgorithm(Algorithm):
    ...
    def __init__(self):
        ...
        self.t0_size: float = 0.0            # T+0 position size
        self.t1_size: float = 0.0            # T+1 position size
        self.t2_size: float = 0.0            # T+2 position size
        self.sell_size: float = 0.0          # Sell position size
        self.pending_sell_pos: float = 0.0  # Track if there is a pending sell position to be executed later
    ...
    def __reset__(self):
        ...
        self._bm_open_size = round_to_lot(self.init_cash // self._init_price, self._stock_lot_size)  # Calculate benchmark shares based on initial cash and current price
    ...
    def __step__(self):
        ...
        if day_diff > 0:
            logging.debug(f"Update T0, T1, T2 for {current_time}, T0: {self.t0_size}, T1: {self.t1_size}, T2: {self.t2_size}, Sell Position: {self.sell_size}")
            # Consecutive prev_day days
            self.sell_size += self.t2_size
            self.t2_size = self.t1_size
            self.t1_size = self.t0_size
            self.t0_size = 0
        ...
        if updated_position == 0:
            ...
            pass
        elif updated_position < 0 or self.pending_sell_pos > 0:
            ...
            if self.sell_size == 0:
                ...
                self.pending_sell_pos = max(self.pending_sell_pos - can_sell_position, 0)  # Reduce pending sell position
                current_action = "S"  # Set action to sell
                current_fee = current_price * current_trade_size * self.init_fee  # Calculate fee based on trade size
